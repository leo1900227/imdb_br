library(pacman)
p_load("tidyverse", "igraph", "tidygraph", "ggraph",network,statnet,reshape2,tidytext,Matrix)
...
# >> 电影-国家矩阵 --------------------------------------------------------------
# https://github.com/juliasilge/tidytext/issues/126

mat_country <- 
  df_country %>%
  count(tt_num, country) %>%
  cast_tdm( tt_num,country, n)
...
# >> 国家-国家矩阵 --------------------------------------------------------------
# https://stackoverflow.com/questions/10622730/create-a-co-occurrence-matrix-from-dummy-coded-observations

mat_cc <- as.matrix(mat_country) %>% crossprod()
...
list_df_cc <- 
  mat_cc %>% 
  graph.adjacency(mode = "undirect",weighted = TRUE) %>%
  get.data.frame() %>% 
  filter(from != to) 


list_df_cc %>% arrange(desc(weight))
